...
elseif(exist('C:\Users\tbaltrus\Documents\DATA\HeadPose', 'file'))
    database_root = 'C:\Users\tbaltrus\Documents\DATA\HeadPose/';    
else
...
%%
v = 3;
[fps_bu_OF, resFolderBU_OF] = run_bu_experiment(buDir, false, v, 'model', 'model/main_dclm_general.txt');
...
% Intensity
v = 4;
[fps_biwi_OF, res_folder_OF] = run_biwi_experiment(database_root, biwi_dir, biwi_results_root, false, false, v, 'model', 'model/main_dclm_general.txt');
...
v = 4;
% Intensity
[fps_ict_OF, res_folder_ict_OF] = run_ict_experiment(database_root, ict_dir, ict_results_root, false, false, v, 'model', 'model/main_dclm_general.txt');
% Calculate the resulting errors
...
%% Save the results
filename = 'results/Pose_OF_dclm';
save(filename);

% Also save them in a reasonable .txt format for easy comparison
f = fopen('results/Pose_OF_dclm.txt', 'w');
