...
for i=1:numel(in_dirs)
    ...
    command = cat(2, command, [' -f "' in_file_name '" -of "' outputFile_fp '" -ov "' outputFile_vid '" -mloc "model/main_dclm_general.txt"']);                     
    dos(command);
end
...
%%
[dclm_error, err_pp_dclm] = compute_error( gts_all - 0.5,  preds_all);
...
[dclm_error49, err_pp_clnf49] = compute_error( gts_all_49 - 0.5,  preds_all_49);

filename = sprintf('results/300VW_DCLM');
save(filename, 'dclm_error', 'dclm_error49');

%%
line_width = 2;

[error_x, error_y] = cummErrorCurve(dclm_error49);
plot(error_x, error_y, 'DisplayName', 'OpenFace-49 DCLM', 'LineWidth',line_width);
hold on;
...
plot(error_x, error_y, 'DisplayName', 'OpenFace-68 DCLM', 'LineWidth',line_width);
hold on;

load('results/300VW_CLNF.mat');
[error_x, error_y] = cummErrorCurve(clnf_error);
plot(error_x, error_y, 'DisplayName', 'OpenFace-68 CLNF', 'LineWidth',line_width);
...
fprintf(f, 'OpenFace (DCLM):  %.4f,   %.4f\n', mean(dclm_error), median(dclm_error));
fprintf(f, 'OpenFace (CLNF):  %.4f,   %.4f\n', mean(clnf_error), median(clnf_error));
fprintf(f, '--------------------------------\n');
fprintf(f, 'Model 49/51, mean,  median\n');
fprintf(f, 'OpenFace (DCLM):  %.4f,   %.4f\n', mean(dclm_error49), median(dclm_error49));
