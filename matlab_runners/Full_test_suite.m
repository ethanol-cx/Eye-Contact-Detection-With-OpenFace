...
assert(median(all_errors_biwi_OF(:)) < 3.25);
assert(median(all_errors_bu_OF(:)) < 2.0);
assert(median(all_errors_ict_OF(:)) < 2.7);
cd('../');
...
assert(median(err_clnf) < 0.059);
run_yt_dataset;
% assert(median(ceclm_error) < 0.048);
assert(median(clnf_error) < 0.064);
...
assert(mean_error < 9.3)
assert(median_error < 8.7)
cd('../');
