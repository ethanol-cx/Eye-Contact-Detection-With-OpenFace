function Script_DCLM_general_vis_select_scale_0()
...
rng(0);
subset = randperm(numel(images), 1000);
...
for reps = 1:500
    ...
    % Removing half of the landmarks
    to_rem = randperm(68, 34);
        
    patches(1).visibilities(to_rem_from,:) = 1;

    patches(1).visibilities(to_rem_from, to_rem) = 0;
    ...
    experiments_full(reps).to_rem = to_rem;
    experiments_full(reps).mean_error = mean(compute_error(labels_all - 0.5, shapes_all));
    experiments_full(reps).median_error = median(compute_error(labels_all - 0.5, shapes_all));

    output_results = 'results/results_wild_dclm_vis_off_scale_0.mat';
